...
{
  await Promise.all(
    REPOS.map(({ repo }) => degit["default"](repo).clone(join(TMP_DIR, id(repo))))
  );
}
...
console.log("Generating src/index.ts file and renaming exports...");
{
  ...
  function generateIndexExports(dir) {
    ...
    for (const file of files) {
      if (file.endsWith("_types.d.ts") || file.endsWith("_types.ts")) {
        continue;
      }
...
// console.log("Generating src/msgs.ts...");
// {
//   const LAST_SEGMENT_REGEX = /[^/]+$/;
//   const EXPORTED_NAME_REGEX = /^export \w+ (\w+) /gm;
//   let contents =
//     "/** This file is generated by gen-protobufs.mjs. Do not edit. */\n\n";
//   contents += `export type AnyMsg = { typeUrl: string; value: unknown };\n\n`;
//   /**
//    * Builds the `src/proto/index.ts` file to re-export generated code.
//    * A prefix is added to the exported names to avoid name collisions.
//    * The prefix is the names of the directories in `proto` leading up
...
//       let contentsFromFile = `export {\n`;
//       let numMsgs = 0;
//       for (const match of code.matchAll(EXPORTED_NAME_REGEX)) {
//         const exportedName = match[1];
//         if (exportedName.startsWith("Msg")) {
//           numMsgs++;
//           contentsFromFile += `  ${exportedName} as ${prefixName + exportedName},\n`;
//         }
//       }
//       const exportedFile = file
//         .replace(PROTOBUFS_DIR + "/", "")
//         .replace(".ts", ".js");
//       contentsFromFile += `} from "./${exportedFile}";\n`;
//       if (numMsgs > 0) {
//         contents += contentsFromFile;
//       }
//     }
//     for (const file of files) {
//       generateMsgs(file);
//     }
//   }
//   generateMsgs(PROTOBUFS_DIR);
//   writeFileSync(join(PROTOBUFS_DIR, "msgs.ts"), contents);
// }
